...
y_cat= y_cat;
%%%---------------Cost Function
prim_sum =sum((-y_cat .* log(third_activator))-((1-y_cat).*log((1-third_activator))));
J = sum(prim_sum)/m;
%%%--------------- END Cost Function---------

%%%---------------Regularized Cost Function

Reg_value = (lambda/(2*m))*(sum(sum((Theta1(:,2:end).^2)))+sum(sum(Theta2(:,2:end).^2)));
...
a_2= [1;a_2]; %agregar factor BIAS
z_3 = Theta2 * a_2;
a_3 = sigmoid(z_3);

err3 = a_3 - y_cat(i,:)';
err2 = ((Theta2(:,2:end)')*err3) .* sigmoidGradient(z2');
Delta1_2 = Delta1_2 +   err3 * a_2';   
Delta1_1 = Delta1_1 +   err2 * activ_1;
